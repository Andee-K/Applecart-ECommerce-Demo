...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const checkoutCountElement = document.querySelector(
    ...
    document.querySelectorAll(".store__add-to-cart").forEach((button) => {
      button.addEventListener("click", function () {
        ...
        const productPrice = parseFloat(
          productElement
            .querySelector(".store__product-price")
            .textContent.replace("$", "")
        );
  ...
  function updateCart() {
    ...
    checkoutCountElement.textContent = cart.reduce(
  ...
  function updateCartPreview() {
    ...
    cart.forEach((item, index) => {
      ...
      // Increase quantity
      li.querySelector(".header__cart-item-increase").addEventListener(
        "click",
        () => {
          item.quantity++;
          updateCart();
      ...
      // Decrease quantity
      li.querySelector(".header__cart-item-decrease").addEventListener(
        "click",
        () => {
          if (item.quantity > 1) {
            item.quantity--;
      ...
      // Remove item
      li.querySelector(".header__cart-item-remove").addEventListener(
        "click",
        () => {
          cart.splice(index, 1);
          showToast(`${item.name} removed from basket`, "error"); // Make sure 'item.name' is used here
